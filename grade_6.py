...
if not os.path.exists(FONT_PATH):
    try:
        ...
        st.error("⚠️ تعذر تحميل الخط العربي تلقائيًا، تأكد من الاتصال بالإنترنت.")
...
# 🔥 تهيئة Firebase (يدعم Local + Streamlit Cloud)
if not firebase_admin._apps:
    try:
        # لو شغال على Streamlit Cloud → يقرأ من secrets
        firebase_config = st.secrets["firebase"]
        cred = credentials.Certificate(firebase_config)
        st.success("✅ تم الاتصال بـ Firebase (Cloud)")
    except Exception:
        # لو شغال محلي → يقرأ من الملف
        PATH_TO_SERVICE_ACCOUNT = "attendance-streamlit-app-c3aa8-firebase-adminsdk-fbsvc-226383f232.json"
        with open(PATH_TO_SERVICE_ACCOUNT) as f:
            firebase_config = json.load(f)
        cred = credentials.Certificate(firebase_config)
        st.info("💻 تم الاتصال بـ Firebase (محلي)")
