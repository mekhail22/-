...
# 🔥 تهيئة Firebase باستخدام st.secrets
...
# 💾 تسجيل الغياب في Firebase بطريقة batch
def record_absences_batch(absent_students, teacher_name):
    ...
    try:
        batch.commit()
        st.success(f"✅ تم تسجيل غياب {len(absent_students)} طالب.")
    except Exception as e:
        st.error(f"⚠️ فشل تسجيل الغياب في Firebase: {e}")
...
# 👨‍🏫 واجهة المدرس
    ...
    if submit:
        if not absent_students:
            ...
            record_absences_batch(absent_students, teacher_name)
